...
void main() {
  ...
  group('planAuthSlate — Supabase', () {
    ...
    test('threads url and anonKey through to the supabase_provider stub', () {
      ...
      final plan = planAuthSlate(backend: 'Supabase', prompt: prompts.call);
      final providerStub =
          plan!.templates.firstWhere((t) => t.name == 'supabase_provider').stub;
  ...
  group('planAuthSlate — Firebase', () {
    test('does not prompt the user (flutterfire configure handles secrets)',
        () {
    ...
    test('schedules firebase_core, firebase_auth and cloud_firestore in order',
        () {
      final prompts = _PromptRecorder(const []);
    ...
    test('emits the Firebase user model template', () {
      ...
      expect(
        plan.templates.any((t) => t.name == 'firebase_provider'),
        isTrue,
      );
    });
  ...
  group('planAuthSlate — unknown backend', () {
    test('returns null without consuming any prompt', () {
      final prompts = _PromptRecorder(const []);
      final plan =
          planAuthSlate(backend: 'NotARealBackend', prompt: prompts.call);
      expect(plan, isNull);
      expect(prompts.asked, isEmpty);
    });
  });
  ...
  group('planIapSlate — RevenueCat', () {
    test('asks for the Apple key first, then the Android key', () {
      ...
      expect(prompts.asked,
          [appleRevenueCatKeyPrompt, androidRevenueCatKeyPrompt]);
    });
    ...
    test('schedules purchases_flutter then purchases_ui_flutter', () {
      ...
      expect(
          plan!.packagesToAdd, ['purchases_flutter', 'purchases_ui_flutter']);
    });
    ...
    test("'n' on both keys leaves both empty (everything is just placeholder)",
        () {
  ...
  group('planIapSlate — unknown service', () {
    test('returns null without consuming any prompt', () {
      final prompts = _PromptRecorder(const []);
      final plan =
          planIapSlate(service: 'NotARealService', prompt: prompts.call);
      expect(plan, isNull);
      expect(prompts.asked, isEmpty);
    });
  });
